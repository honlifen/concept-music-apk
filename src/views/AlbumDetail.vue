<template>
  <div class="h-full flex flex-col overflow-y-auto">
    <div v-if="info" class="relative">
      <div class="h-44 bg-gradient-to-b from-green-100 to-white dark:from-green-950/30 dark:to-slate-950 flex items-end p-4">
        <div class="flex gap-4 items-end">
          ...
          <div class="flex-1 min-w-0 pb-1">
            <h2 class="text-lg font-extrabold leading-snug">{{ info.name }}</h2>
            <p class="text-xs text-gray-500 mt-1">{{ info.singer || '' }}</p>
            <p class="text-[11px] text-gray-400 mt-0.5">{{ info.publish_date || '' }}</p>
          </div>
        </div>
      </div>

      <div class="px-4 py-3">
        <button @click="playAll" class="flex items-center gap-1.5 px-5 py-2.5 bg-green-600 text-white rounded-full text-sm font-bold shadow-lg shadow-green-200 tap-scale active:scale-95">
...
onMounted(async () => {
  ...
  try {
    const res = await request.get('/album/info', { params: { albumid: id, page: 1, pagesize: 30 }, silent: true })
    const data = res?.data || {}
    info.value = {
      name: data.albumname || data.album || data.name || '',
      pic: data.imgurl || data.pic || '',
      singer: data.singername || data.singer || '',
      publish_date: data.publish_date || data.public_time || '',
    }
    songs.value = (data.song_list || data.songs || []).map(s => ({
      ...
      singer: info.value.singer || s.SingerName || s.singername || '',
      cover: s.Cover || s.cover || '',
    })).filter(s => s.hash)
  } catch (e) { console.error('[AlbumDetail]', e.message) }
